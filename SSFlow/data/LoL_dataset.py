...
class LoL_Dataset(data.Dataset):
    ...
    def load_pairs(self, folder_path):
        ...
        # low_list = os.listdir(os.path.join(folder_path,'low',low_list[0]))
        low_list= glob.glob(folder_path+'/low'+'/*/'+"*.png")
        low_list = filter(lambda x: 'png' in x, low_list)
        low_list1 = glob.glob(folder_path + '/high' + '/*/' + "*.png")
        low_list1 = filter(lambda x: 'png' in x, low_list1)
        pairs = []
        for idx, f_name in enumerate(low_list):
            ...
            #      # [:, 4:-4, :],
            #      f_name.split('.')[0]])
            f_name1=f_name.replace("low", "high")
            pairs.append(
                [cv2.cvtColor(cv2.imread(f_name), cv2.COLOR_BGR2RGB),
                 cv2.cvtColor(cv2.imread(f_name1), cv2.COLOR_BGR2RGB),f_name.split('.')[0]])
            # if idx > 10: break
