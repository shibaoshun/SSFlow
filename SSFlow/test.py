...
import os
import cv2
# To conduct tests, must first download the pretrained models and the relevant datasets.
# We have placed some of the test results in the results folder.
...
def main():
    ...
    test_dir = os.path.join(this_dir, '..', 'resultsgithub1', conf)
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        sr_t = torch.where(torch.isnan(sr_t), torch.full_like(sr_t, 1), sr_t)
