...
#from TV_denoising import TV_denoising
class LLFlow(nn.Module):
    ...
    def normal_flow(self, gt, lr, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None,
        ...
        # gt1=gt1.transpose(0,2,3,1)
        # #gt1=torch.from_numpy(gt1)
        # gt1 = denoise_tv_chambolle(gt1, weight=0.1, eps=0.0002, n_iter_max=200, multichannel=True)
        # tv=torch.from_numpy(gt1).cuda()
        # tv=tv.permute(0,3,1,2)
        ...
        if 'avg_color_map' in self.opt.keys() and self.opt['avg_color_map']:
            if 'avg_pool_color_map' in self.opt.keys() and self.opt['avg_pool_color_map']:
                ...
                #     'train_gt_ratio'] else squeeze2d(F.avg_pool2d(gt / (gt.sum(dim=1, keepdims=True) + 1e-4), 7, 1, 3), 8)
                # mean = squeeze2d(F.avg_pool2d(lr_enc['color_map'], 7, 1, 3), 8) if random.random() > self.opt[
                #     'train_gt_ratio'] else squeeze2d(F.avg_pool2d(gt, 7, 1, 3),
                #                                      8)
                mean = squeeze2d(F.avg_pool2d(lr_enc['color_map'], 7, 1, 3), 8) if random.random() > self.opt[
                    'train_gt_ratio'] else squeeze2d(F.avg_pool2d(gt / (gt.sum(dim=1, keepdims=True) + 1e-4), 7, 1, 3),
                                                     8)
        else:
            if self.RRDB is not None:
                mean = squeeze2d(lr_enc['color_map'], 8) if random.random() > self.opt['train_gt_ratio'] else squeeze2d(F.avg_pool2d(gt / (gt.sum(dim=1, keepdims=True) + 1e-4), 7, 1, 3), 8)
            else:
    ...
    def reverse_flow(self, lr, z, y_onehot, eps_std, epses=None, lr_enc=None, add_gt_noise=True):
        ...
        # gt1=x.cpu().detach().numpy()
        # gt1=gt1.transpose(0,2,3,1)
        # gt1 = denoise_tv_chambolle(gt1, weight=0.1, eps=0.0002, max_num_iter=200, channel_axis=True)
        # tv=torch.from_numpy(gt1).cuda()
        # x=tv.permute(0,3,1,2)
        return x, logdet
